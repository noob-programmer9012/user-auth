...
export async function editTransporter(req, res, next) {
  ...
  if (transporter) {
    try {
      ...
      // if (
      //   (await Transporter.findOne({ transporterName })) &&
      //   Transporter.findOne({ gst_no })
      // ) {
      //   return next(new ErrorResponse("Duplicate Entry", 400));
      // }
      const duplicateTrans = await Transporter.findOne({
        firmId,
        transporterName,
      });
      const duplicateGst = await Transporter.findOne({ firmId, gst_no });
      if (duplicateGst && duplicateTrans) {
        return next(new ErrorResponse("Duplicate Entry", 400));
      }
